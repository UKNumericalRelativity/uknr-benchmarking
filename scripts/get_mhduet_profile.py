#!/usr/bin/env python3
"""
Parse MHDuet timer output files and produce a profile summary.

Reads .out files from 3 runs of a given job size, extracts walltimes
for selected timers from the exclusive timer table, uses AmrCoreProblem::Evolve()
...
BUCKET_PATTERNS = [
    # RHS
    (lambda name: name == "AmrCoreProblem::AdvanceLevel: RHS", "RHS"),
    # Sync and Comms
    ...
    (lambda name: name == "AmrCoreProblem::AverageDownTo(crse_lev)", "Sync and Comms"),
    (lambda name: name == "AmrCoreProblem::FillPatch()", "Sync and Comms"),
    (lambda name: name == "FabArray::FillBoundaryAndSync()", "Sync and Comms"),
]

# Base directories for MHDuet submit outputs
COSMA8_DIR = Path(__file__).parents[1] / "codes" / "MHDuet" / "COSMA8" / "submit"
TURSA_DIR = Path(__file__).parents[1] / "codes" / "MHDuet" / "Tursa" / "submit"
...
def parse_timer_tables(output_path):
    """
    Parse the two timer tables from a MHDuet output file.
    ...
    for line in exclusive_block:
        ...
        # Timer names may contain spaces (e.g., "AmrCoreProblem::AdvanceLevel: RHS")
        # Parse from the right: last 5 columns are numeric, rest is timer name
        # From right: Max% is -1, Max is -2, Avg is -3, Min is -4, NCalls is -5
        parts = line.split()
        if len(parts) >= 6:
            try:
                ncalls = int(parts[-5])
                excl_avg = float(parts[-3])
                timer_name = " ".join(parts[:-5])
...
def main():
    parser = argparse.ArgumentParser(
        description="Parse MHDuet timer outputs and produce a profile summary."
    )
    parser.add_argument(
        "job_size",
        type=str,
    ...
    # Total time from AmrCoreProblem::Evolve() in inclusive table (averaged)
    total_times = [t.get("AmrCoreProblem::Evolve()", 0.0) for t in all_inclusive]
    total_avg = sum(total_times) / len(total_times)

    if total_avg == 0.0:
        print("Error: 'AmrCoreProblem::Evolve()' timer not found in any run.")
        return
    ...
    output_path = Path(__file__).parent / f"mhduet_{system}_{job}_profile_summary.dat"
